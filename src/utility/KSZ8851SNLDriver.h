...
class KSZ8851SNLCallbackDriver : public KSZ8851SNLDriver {
public:

  KSZ8851SNLCallbackDriver(uint16_t poll_interval_ms = 60, uint32_t sw_reset_timeout_ms = 300)
      : KSZ8851SNLDriver(-1, -1, -1), poll_interval(poll_interval_ms > 0 ? poll_interval_ms : 60), sw_reset_timeout(sw_reset_timeout_ms > 0 ? sw_reset_timeout_ms : 300) {
  }
